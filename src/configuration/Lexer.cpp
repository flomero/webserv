...
Token Lexer::parseKeywordOrString() {
    ...
    if (value == "listen")					return {TOKEN_LISTEN, value, _line, _column};
    ...
    if (value == "client_max_body_size")	return {TOKEN_CLIENT_BODY_SIZE, value, _line, _column};
    if (value == "upload_dir")				return {TOKEN_UPLOAD_DIR, value, _line, _column};
    if (value == "request_timeout")			return {TOKEN_REQUEST_TIMEOUT, value, _line, _column};
    if (value == "error_page")				return {TOKEN_ERROR_PAGE, value, _line, _column};
    if (value == "allow_methods")			return {TOKEN_ALLOW_METHODS, value, _line, _column};
    if (value == "autoindex")				return {TOKEN_AUTOINDEX, value, _line, _column};
    if (value == "alias")					return {TOKEN_ALIAS, value, _line, _column};
    if (value == "cgi")						return {TOKEN_CGI, value, _line, _column};
    if (value == "return")					return {TOKEN_RETURN, value, _line, _column};
