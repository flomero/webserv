...
void NoOpChecker(void*) { return; }

void GreenletChecker(void* p) {
	if (!p) {
		return;
	}
...
void MainGreenletExactChecker(void* p);
...
void ContextExactChecker(void* p) {
	if (!p) {
		return;
	}
	if (!PyContext_CheckExact(p)) {
		throw TypeError("greenlet context must be a contextvars.Context or None");
	}
}
...
class PyObjectPointer {
	...
	public:
		PyObjectPointer(T* it = nullptr) : p(it) { TC(p); }
		...
		T* borrow() const noexcept { return this->p; }

		PyObject* borrow_o() const noexcept { return reinterpret_cast<PyObject*>(this->p); }

		T* operator->() const noexcept { return this->p; }

		bool is_None() const noexcept { return this->p == Py_None; }
		...
		bool operator!() const noexcept { return this->p == nullptr; }

		Py_ssize_t REFCNT() const noexcept { return p ? Py_REFCNT(p) : -42; }

		PyTypeObject* TYPE() const noexcept { return p ? Py_TYPE(p) : nullptr; }
		...
		inline OwnedObject PyCall(const BorrowedObject args, const BorrowedObject kwargs) const;
		inline OwnedObject PyCall(const OwnedObject& args, const OwnedObject& kwargs) const;
	...
	protected:
		...
		void* _get_raw_pointer() const { return p; }
};
...
std::ostream& operator<<(std::ostream& os, const PyObjectPointer<T, TC>& s) {
	...
	os << t.name() << "(addr=" << s.borrow() << ", refcnt=" << s.REFCNT() << ", value=" << s.as_str() << ")";
...
class OwnedReference : public PyObjectPointer<T, TC> {
	...
	public:
		...
		static OwnedReference<T, TC> consuming(PyObject* p) { return OwnedReference<T, TC>(reinterpret_cast<T*>(p)); }
		...
		explicit OwnedReference(const PyObjectPointer<>& other) : PyObjectPointer<T, TC>(nullptr) {
			T* op = other.borrow();
			TC(op);
			this->p = other.borrow();
			Py_XINCREF(this->p);
		...
		OwnedReference(const OwnedReference<T, TC>& other) : PyObjectPointer<T, TC>(other.p) { Py_XINCREF(this->p); }
		...
		~OwnedReference() { Py_CLEAR(this->p); }
...
static inline void operator<<=(PyObject*& target, OwnedObject& o) { target = o.relinquish_ownership(); }
...
class NewDictReference : public NewReference {
	...
	public:
		...
		void SetItem(const char* const key, PyObject* value) { Require(PyDict_SetItemString(this->p, key, value)); }
...
class _OwnedGreenlet : public OwnedReference<T, TC> {
	...
	public:
		...
		_OwnedGreenlet(OwnedMainGreenlet& other) : OwnedReference<T, TC>(reinterpret_cast<T*>(other.acquire())) {}
		...
		inline _OwnedGreenlet<T, TC>& operator=(const OwnedGreenlet& other) { return this->operator=(other.borrow()); }
		...
		PyObject* relinquish_ownership_o() { return reinterpret_cast<PyObject*>(relinquish_ownership()); }
...
class _BorrowedGreenlet : public BorrowedReference<T, TC> {
	public:
		_BorrowedGreenlet() : BorrowedReference<T, TC>(nullptr) {}

		_BorrowedGreenlet(T* it) : BorrowedReference<T, TC>(it) {}

		_BorrowedGreenlet(const BorrowedObject& it);

		_BorrowedGreenlet(const OwnedGreenlet& it) : BorrowedReference<T, TC>(it.borrow()) {}

		_BorrowedGreenlet<T, TC>& operator=(const BorrowedObject& other);

		// We get one of these for PyGreenlet, but one for PyObject
		// is handy as well
		operator PyObject*() const { return reinterpret_cast<PyObject*>(this->p); }
...
class BorrowedMainGreenlet : public _BorrowedGreenlet<PyGreenlet, MainGreenletExactChecker> {
	public:
		BorrowedMainGreenlet(const OwnedMainGreenlet& it)
			: _BorrowedGreenlet<PyGreenlet, MainGreenletExactChecker>(it.borrow()) {}

		BorrowedMainGreenlet(PyGreenlet* it = nullptr) : _BorrowedGreenlet<PyGreenlet, MainGreenletExactChecker>(it) {}
};
...
class ImmortalObject : public PyObjectPointer<> {
	...
	public:
		...
		ImmortalObject(const ImmortalObject& other) : PyObjectPointer<>(other.p) {}
		...
		static ImmortalObject consuming(PyObject* it) { return ImmortalObject(it); }

		inline operator PyObject*() const { return this->p; }
};
...
class ImmortalException : public ImmortalObject {
	...
	public:
		...
		inline bool PyExceptionMatches() const { return PyErr_ExceptionMatches(this->p) > 0; }
};
...
inline OwnedObject PyObjectPointer<T, TC>::PyRequireAttr(const ImmortalString& name) const {
	assert(this->p);
	return OwnedObject::consuming(Require(PyObject_GetAttr(reinterpret_cast<PyObject*>(this->p), name), name));
}
...
template <typename T, TypeChecker TC>
inline OwnedObject PyObjectPointer<T, TC>::PyCall(const BorrowedObject args, const BorrowedObject kwargs) const {
	assert(this->p);
	return OwnedObject::consuming(PyObject_Call(this->p, args, kwargs));
}

template <typename T, TypeChecker TC>
inline OwnedObject PyObjectPointer<T, TC>::PyCall(const OwnedObject& args, const OwnedObject& kwargs) const {
	assert(this->p);
	return OwnedObject::consuming(PyObject_Call(this->p, args.borrow(), kwargs.borrow()));
}

inline void ListChecker(void* p) {
	if (!p) {
		return;
	}
	if (!PyList_Check(p)) {
...
class OwnedList : public OwnedReference<PyObject, ListChecker> {
	...
	public:
		...
		explicit OwnedList(const OwnedObject& other) : OwnedReference<PyObject, ListChecker>(other) {}
		...
		inline bool empty() const { return PyList_GET_SIZE(p) == 0; }

		inline Py_ssize_t size() const { return PyList_GET_SIZE(p); }

		inline BorrowedObject at(const Py_ssize_t index) const { return PyList_GET_ITEM(p, index); }

		inline void clear() { PyList_SetSlice(p, 0, PyList_GET_SIZE(p), NULL); }
};
...
class CreatedModule : public PyObjectPointer<> {
	...
	public:
		CreatedModule(PyModuleDef& mod_def) : PyObjectPointer<>(Require(PyModule_Create(&mod_def))) {}
...
class OwnedErrPiece : public OwnedObject {
	...
	public:
		...
		OwnedErrPiece(PyObject* p = nullptr) : OwnedObject(p) { this->acquire(); }

		PyObject** operator&() { return &this->p; }

		inline operator PyObject*() const { return this->p; }

		operator PyTypeObject*() const { return reinterpret_cast<PyTypeObject*>(this->p); }
};
...
class PyErrPieces {
	...
	public:
		...
		PyErrPieces(PyObject* t, PyObject* v, PyObject* tb) : type(t), instance(v), traceback(tb), restored(0) {
			this->normalize();
		}

		PyErrPieces() : restored(0) {
		...
		void PyErrRestore() {
			...
			this->restored = true;
			PyErr_Restore(this->type.relinquish_ownership(), this->instance.relinquish_ownership(),
	...
	private:
		void normalize() {
			...
			if (traceback && !PyTraceBack_Check(traceback.borrow())) {
				throw PyErrOccurred(PyExc_TypeError, "throw() third argument must be a traceback object");
			}
			...
			if (PyExceptionClass_Check(type)) {
				...
				if (instance && !instance.is_None()) {
					throw PyErrOccurred(PyExc_TypeError, "instance exception may not have a separate value");
				}
...
class PyArgParseParam : public BorrowedObject {
	...
	public:
		...
		inline PyObject** operator&() { return &this->p; }
};
