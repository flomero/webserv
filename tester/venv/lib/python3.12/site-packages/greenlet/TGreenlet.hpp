...
class PythonStateContext {
	...
	public:
		inline const greenlet::refs::OwnedContext& context() const { return this->_context; }

		inline greenlet::refs::OwnedContext& context() { return this->_context; }

		inline void tp_clear() { this->_context.CLEAR(); }
...
class SwitchingArgs {
	...
	public:
		...
		SwitchingArgs(const OwnedObject& args, const OwnedObject& kwargs) : _args(args), _kwargs(kwargs) {}

		SwitchingArgs(const SwitchingArgs& other) : _args(other._args), _kwargs(other._kwargs) {}

		const OwnedObject& args() { return this->_args; }

		const OwnedObject& kwargs() { return this->_kwargs; }
		...
		explicit operator bool() const noexcept { return this->_args || this->_kwargs; }
		...
		const std::string as_str() const noexcept {
			return PyUnicode_AsUTF8(
				OwnedObject::consuming(PyUnicode_FromFormat("SwitchingArgs(args=%R, kwargs=%R)", this->_args.borrow(),
															this->_kwargs.borrow()))
					.borrow());
		}
};
...
class Greenlet {
	...
	public:
		...
		inline void may_switch_away() { this->python_state.may_switch_away(); }

		inline void context(refs::BorrowedObject new_context);

		inline SwitchingArgs& args() { return this->switch_args; }

		virtual const refs::BorrowedMainGreenlet main_greenlet() const = 0;

		inline intptr_t stack_saved() const noexcept { return this->stack_state.stack_saved(); }
		...
		// simplest way.
		inline const char* stack_start() const noexcept { return this->stack_state.stack_start(); }
		...
		inline bool started() const { return this->stack_state.started(); }

		inline bool active() const { return this->stack_state.active(); }

		inline bool main() const { return this->stack_state.main(); }
		...
		inline const PythonState::OwnedFrame& top_frame() { return this->python_state.top_frame(); }
		...
		inline BorrowedGreenlet self() const noexcept { return BorrowedGreenlet(this->_self); }
	...
	protected:
		...
		struct switchstack_result_t {
				...
				switchstack_result_t() : status(0), the_new_current_greenlet(nullptr) {}

				switchstack_result_t(int err) : status(err), the_new_current_greenlet(nullptr) {}

				switchstack_result_t(int err, Greenlet* state, OwnedGreenlet& origin)
					: status(err), the_new_current_greenlet(state), origin_greenlet(origin) {}
		...
		OwnedObject on_switchstack_or_initialstub_failure(Greenlet* target, const switchstack_result_t& err,
	...
	protected:
		...
		*/
		...
		class TracingGuard {
			...
			public:
				TracingGuard() : tstate(PyThreadState_GET()) { PyThreadState_EnterTracing(this->tstate); }
				...
				inline void CallTraceFunction(const OwnedObject& tracefunc,
											  ...
											  const BorrowedGreenlet& origin, const BorrowedGreenlet& target) {
					...
					assert(origin);
					assert(target);
					greenlet::refs::NewReference retval(PyObject_CallFunction(
						tracefunc.borrow(), "O(OO)", event.borrow(), origin.borrow(), target.borrow()));
					if (!retval) {
		...
		static void g_calltrace(const OwnedObject& tracefunc, const greenlet::refs::ImmortalEventName& event,
								const greenlet::refs::BorrowedGreenlet& origin, const BorrowedGreenlet& target);
...
class BrokenGreenlet : public UserGreenlet {
	...
	public:
		...
		BrokenGreenlet(PyGreenlet* p, BorrowedGreenlet the_parent) : UserGreenlet(p, the_parent) {}
...
class GCDisabledGuard {
	...
	public:
		GCDisabledGuard() : was_enabled(PyGC_IsEnabled()) { PyGC_Disable(); }
...
static inline OwnedObject single_result(const OwnedObject& results) {
	if (results && PyTuple_Check(results.borrow()) && PyTuple_GET_SIZE(results.borrow()) == 1) {
...
static OwnedObject g_handle_exit(const OwnedObject& greenlet_result);
