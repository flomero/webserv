...
void GREENLET_NOINLINE(PythonState::unexpose_frames)() {
	...
	while (iframe != nullptr) {
		...
		memcpy(&iframe->previous, &iframe->frame_obj->_f_frame_data[0], sizeof(void *));
		iframe = prev_exposed;
	}
}
#else
...
const PythonState::OwnedFrame &PythonState::top_frame() const noexcept { return this->_top_frame; }
