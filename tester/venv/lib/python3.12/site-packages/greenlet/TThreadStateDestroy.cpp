...
struct ThreadState_DestroyNoGIL {
	...
	private:
		...
		// - return 1
		// Otherwise, return 0.
		static bool MarkGreenletDeadIfNeeded(ThreadState* const state) {
		...
		static void AddToCleanupQueue(ThreadState* const state) {
			...
			if (mod_globs->thread_states_to_destroy.size() == 1) {
				...
				int result = AddPendingCall(PendingCallback_DestroyQueueWithGIL, nullptr);
				if (result < 0) {
		...
		static int PendingCallback_DestroyQueueWithGIL(void* UNUSED(arg)) {
		...
		static void DestroyOneWithGIL(const ThreadState* const state) {
			// Holding the GIL.
		...
		static int _push_pending_call(struct _pending_calls* pending, int (*func)(void*), void* arg) {
			int i = pending->last;
...
// offset.
typedef greenlet::ThreadStateCreator<greenlet::ThreadState_DestroyNoGIL::MarkGreenletDeadAndQueueCleanup>
	ThreadStateCreator;
