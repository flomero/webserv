...
static PyGreenlet* green_new(PyTypeObject* type, PyObject* UNUSED(args), PyObject* UNUSED(kwds)) {
	PyGreenlet* o = (PyGreenlet*)PyBaseObject_Type.tp_new(type, mod_globs->empty_tuple, mod_globs->empty_dict);
	if (o) {
...
static int green_init(PyGreenlet* self, PyObject* args, PyObject* kwargs) {
	...
	static const char* kwlist[] = {"run", "parent", NULL};

	// recall: The O specifier does NOT increase the reference count.
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|OO:green", (char**)kwlist, &run, &nparent)) {
		return -1;
	}
...
static int green_is_gc(PyObject* _self) {
 ...
 **/
static int _green_dealloc_kill_started_non_main_greenlet(BorrowedGreenlet self) {
	...
	try {
		...
		self->deallocing_greenlet_in_thread(self->thread_state() ? static_cast<ThreadState*>(GET_THREAD_STATE())
																 : nullptr);
...
static int green_bool(PyGreenlet* self) { return self->pimpl->active(); }

/**
 * CAUTION: Allocates memory, may run GC and arbitrary Python code.
 */
static PyObject* green_getdict(PyGreenlet* self, void* UNUSED(context)) {
	if (self->dict == NULL) {
...
static int green_setdict(PyGreenlet* self, PyObject* val, void* UNUSED(context)) {
	PyObject* tmp;
...
static bool _green_not_dead(BorrowedGreenlet self) {
...
static PyObject* green_get_stack_saved(PyGreenlet* self, void* UNUSED(context)) {
...
static PyObject* green_getcontext(const PyGreenlet* self, void* UNUSED(context)) {
...
static PyObject* green_repr(PyGreenlet* _self) {
	BorrowedGreenlet self(_self);
	/*
	...
	if (_green_not_dead(self)) {
		...
		*/
		...
		if (self->was_running_in_dead_thread()) {
			...
			state_in_thread =
				GET_THREAD_STATE().state().is_current(self) ? " current" : (self->started() ? " suspended" : "");
		}
		result = PyUnicode_FromFormat("<%s object at %p (otid=%p)%s%s%s%s>", tp_name, self.borrow_o(),
									  self->thread_state(), state_in_thread, self->active() ? " active" : "",
									  never_started ? " pending" : " started", self->main() ? " main" : "");
	} else {
		result =
			PyUnicode_FromFormat("<%s object at %p (otid=%p) %sdead>", tp_name, self.borrow_o(), self->thread_state(),
								 self->was_running_in_dead_thread() ? "(thread exited) " : "");
	}
...
static PyGetSetDef green_getsets[] = {
	...
	{.name = "gr_context", .get = (getter)green_getcontext, .set = (setter)green_setcontext},
	{.name = "dead", .get = (getter)green_getdead},
	{.name = "_stack_saved", .get = (getter)green_get_stack_saved},
	{.name = NULL}};

static PyMemberDef green_members[] = {{.name = NULL}};
...
PyTypeObject PyGreenlet_Type = {
	.ob_base = PyVarObject_HEAD_INIT(NULL, 0).tp_name = "greenlet.greenlet", /* tp_name */
