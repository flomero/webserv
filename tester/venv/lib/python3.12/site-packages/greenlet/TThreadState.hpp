...
class ThreadState {
	...
	public:
		static void* operator new(size_t UNUSED(count)) { return ThreadState::allocator.allocate(1); }

		static void operator delete(void* ptr) {
			return ThreadState::allocator.deallocate(static_cast<ThreadState*>(ptr), 1);
		}
		...
		ThreadState() {
			...
			this->main_greenlet = OwnedMainGreenlet::consuming(main->self());
		...
		inline bool has_main_greenlet() const noexcept { return bool(this->main_greenlet); }
		...
		inline OwnedMainGreenlet get_main_greenlet() const noexcept { return this->main_greenlet; }
		 ...
		 * Does no maintenance.
		 */
		inline OwnedGreenlet get_current() const { return this->current_greenlet; }
		...
		inline void set_current(const OwnedGreenlet& target) { this->current_greenlet = target; }
	...
	private:
		...
		inline void clear_deleteme_list(const bool murder = false) {
			if (!this->deleteme.empty()) {
				...
				for (deleteme_t::iterator it = copy.begin(), end = copy.end(); it != end; ++it) {
	...
	public:
		 ...
		 */
		inline OwnedObject get_tracefunc() const { return tracefunc; };
		 ...
		 */
		inline static std::clock_t& clocks_used_doing_gc() { return ThreadState::_clocks_used_doing_gc; }
		...
		~ThreadState() {
			...
			if (this->current_greenlet == this->main_greenlet && this->current_greenlet) {
				...
				if (ThreadState::_clocks_used_doing_gc != std::clock_t(-1) && cnt == 2 &&
					Py_REFCNT(old_main_greenlet) == 1) {
					...
					if (gc) {
						...
						} else if (refs && refs.size() == 1 && PyCFunction_Check(refs.at(0)) &&
