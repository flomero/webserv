...
Greenlet::Greenlet(PyGreenlet* p) : Greenlet(p, StackState()) {}

Greenlet::Greenlet(PyGreenlet* p, const StackState& initial_stack) : _self(p), stack_state(initial_stack) {
...
bool Greenlet::force_slp_switch_error() const noexcept { return false; }

void Greenlet::release_args() { this->switch_args.CLEAR(); }

/**
 * CAUTION: This will allocate memory and may trigger garbage
 * collection and arbitrary Python code.
 */
OwnedObject Greenlet::throw_GreenletExit_during_dealloc(const ThreadState& UNUSED(current_thread_state)) {
...
inline void Greenlet::slp_restore_state() noexcept {
#ifdef SLP_BEFORE_RESTORE_STATE
	SLP_BEFORE_RESTORE_STATE();
#endif
	this->stack_state.copy_heap_to_stack(this->thread_state()->borrow_current()->stack_state);
}

inline int Greenlet::slp_save_state(char* const stackref) noexcept {
...
#endif
	return this->stack_state.copy_stack_to_heap(stackref, this->thread_state()->borrow_current()->stack_state);
}
 ...
 * collection and arbitrary Python code.
 */
OwnedObject Greenlet::on_switchstack_or_initialstub_failure(Greenlet* target, const Greenlet::switchstack_result_t& err,
															const bool target_was_me, const bool was_initial_stub) {
	...
	if (!PyErr_Occurred()) {
		PyErr_SetString(PyExc_SystemError, was_initial_stub
...
OwnedGreenlet Greenlet::g_switchstack_success() noexcept {
...
Greenlet::switchstack_result_t Greenlet::g_switchstack(void) {
	...
	{ /* save state */
		if (this->thread_state()->is_current(this->self())) {
			...
			// important?
			return switchstack_result_t(0, this, this->thread_state()->borrow_current());
		}
...
inline void Greenlet::check_switch_allowed() const {
	...
	if (!main_greenlet) {
		throw PyErrOccurred(mod_globs->PyExc_GreenletError, "cannot switch to a garbage collected greenlet");
	}
...
const OwnedObject Greenlet::context() const {
...
OwnedObject Greenlet::g_switch_finish(const switchstack_result_t& err) {
	...
	try {
		...
		if (OwnedObject tracefunc = state.get_tracefunc()) {
			...
			g_calltrace(tracefunc, result ? mod_globs->event_switch : mod_globs->event_throw, err.origin_greenlet,
						this->self());
		}
...
void Greenlet::g_calltrace(const OwnedObject& tracefunc, const greenlet::refs::ImmortalEventName& event,
	...
	assert((event == mod_globs->event_throw && PyErr_Occurred()) ||
...
inline void Greenlet::deactivate_and_free() {
	if (!this->active()) {
		return;
	}
...
bool Greenlet::belongs_to_thread(const ThreadState* thread_state) const {
...
void Greenlet::deallocing_greenlet_in_thread(const ThreadState* current_thread_state) {
...
int Greenlet::tp_traverse(visitproc visit, void* arg) {
	int result;
...
void GREENLET_NOINLINE(Greenlet::expose_frames)() {
	...
	while (iframe) {
		...
		if (!_PyFrame_IsIncomplete(&iframe_copy)) {
			...
			assert(iframe->owner == FRAME_OWNED_BY_THREAD || iframe->owner == FRAME_OWNED_BY_GENERATOR);
			if (last_complete_iframe) {
				assert(last_complete_iframe->frame_obj);
				memcpy(&last_complete_iframe->frame_obj->_f_frame_data[0], &last_complete_iframe->previous,
					   sizeof(void*));
	...
	if (last_complete_iframe) {
		assert(last_complete_iframe->frame_obj);
		memcpy(&last_complete_iframe->frame_obj->_f_frame_data[0], &last_complete_iframe->previous, sizeof(void*));
